namespace SpaceEngineers.Core.DataAccess.Orm.Sql.Migrations.Postgres.Internals
{
    ...
    {
        private const string InsertFunctionCommandFormat = $@"insert into ""{nameof(Migrations)}"".""{nameof(FunctionView)}""(""{nameof(FunctionView.PrimaryKey)}"", ""{nameof(FunctionView.Version)}"", ""{nameof(FunctionView.Schema)}"", ""{nameof(FunctionView.Function)}"", ""{nameof(FunctionView.Definition)}"") values (@param_0, @param_1, @param_2, @param_3, @param_4)";
