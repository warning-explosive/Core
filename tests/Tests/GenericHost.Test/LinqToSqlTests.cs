namespace SpaceEngineers.Core.GenericHost.Test
{
    ...
    public class LinqToSqlTests : TestBase
    {
        ...
        public LinqToSqlTests(ITestOutputHelper output, TestFixture fixture)
            : base(output, fixture)
        {
        ...
        /// <summary>
        /// Test cases for LinqToSql tests
        /// </summary>
        /// <returns>Test data</returns>
        public static IEnumerable<object[]> LinqToSqlTestData()
        {
        ...
        internal static IEnumerable<object[]> CommandTranslationTestCases()
        {
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}b.""{nameof(DatabaseEntity.NullableStringField)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.NullableStringField)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntity)}"" a) b{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN @param_0 IS NULL THEN b.""{nameof(DatabaseEntity.NullableStringField)}"" IS NOT NULL ELSE b.""{nameof(DatabaseEntity.NullableStringField)}"" != @param_1 END",
            ...
            yield return new object[]
            {
                $"{nameof(DataAccess.Orm.Sql.Postgres)} - binary filter (reverse)",
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}b.""StringField"",{Environment.NewLine}{'\t'}b.""Filter""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.StringField)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.NullableStringField)}"" AS ""Filter""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntity)}"" a) b{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN CASE WHEN @param_0 IS NULL THEN b.""Filter"" IS NOT NULL ELSE b.""Filter"" != @param_1 END THEN @param_2 ELSE @param_3 END",
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}b.""{nameof(DatabaseEntity.StringField)}"",{Environment.NewLine}{'\t'}b.""{nameof(DatabaseEntity.NullableStringField)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.StringField)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.NullableStringField)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntity)}"" a) b{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN CASE WHEN @param_0 IS NULL THEN b.""{nameof(DatabaseEntity.NullableStringField)}"" IS NOT NULL ELSE b.""{nameof(DatabaseEntity.NullableStringField)}"" != @param_1 END THEN @param_2 ELSE @param_3 END",
                        new[] { new SqlCommandParameter("param_0", default(string), typeof(string)), new SqlCommandParameter("param_1", default(string), typeof(string)), new SqlCommandParameter("param_2", true, typeof(bool)), new SqlCommandParameter("param_3", false, typeof(bool)) },
                        log)),
                new IDatabaseEntity[] { databaseEntity }
            };
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT DISTINCT{Environment.NewLine}{'\t'}e.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Blog.PrimaryKey)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Blog.Theme)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Blog.Version)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.Nickname)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(AuthEndpoint.Domain.User.Version)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}{'\t'}""{schema}"".""{nameof(Post)}"" a{Environment.NewLine}{'\t'}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}{'\t'}""{schema}"".""{nameof(Blog)}"" b{Environment.NewLine}{'\t'}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Blog.PrimaryKey)}"" = a.""{nameof(Post.Blog)}""{Environment.NewLine}{'\t'}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntities.Relations.User)}"" c{Environment.NewLine}{'\t'}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" = a.""{nameof(Post.User)}"") d{Environment.NewLine}{'\t'}WHERE{Environment.NewLine}{'\t'}{'\t'}CASE WHEN @param_0 IS NULL THEN d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"" IS NULL ELSE d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"" = @param_1 END) e",
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}b.""Nsf"",{Environment.NewLine}{'\t'}b.""Sf""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.NullableStringField)}"" AS ""Nsf"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(DatabaseEntity.StringField)}"" AS ""Sf""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntity)}"" a) b{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN @param_0 IS NULL THEN b.""Nsf"" IS NOT NULL ELSE b.""Nsf"" != @param_1 END",
                        new[] { new SqlCommandParameter("param_0", default(string), typeof(string)), new SqlCommandParameter("param_1", default(string), typeof(string)) },
                        log)),
                new IDatabaseEntity[] { databaseEntity }
            };
            ...
            yield return new object[]
            {
                $"{nameof(DataAccess.Orm.Sql.Postgres)} - one-to-one relation in order by",
            ...
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}d.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}a.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Blog.PrimaryKey)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Blog.Theme)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Blog.Version)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.Nickname)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(AuthEndpoint.Domain.User.Version)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(Post)}"" a{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(Blog)}"" b{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Blog.PrimaryKey)}"" = a.""{nameof(Post.Blog)}""{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntities.Relations.User)}"" c{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" = a.""{nameof(Post.User)}"") d{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN @param_0 IS NULL THEN d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"" IS NULL ELSE d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"" = @param_1 END AND CASE WHEN @param_2 IS NULL THEN d.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"" IS NULL ELSE d.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"" = @param_3 END",
            ...
            yield return new object[]
            {
                $"{nameof(DataAccess.Orm.Sql.Postgres)} - one-to-one relation in filter after projection",
                new Func<IDependencyContainer, object?>(container => container.Resolve<IDatabaseContext>().All<Post>().Select(it => new { it.Text, it.Blog, it.User }).Where(it => it.Blog.Theme == "MilkyWay")),
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.PrimaryKey)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.Theme)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.Version)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.Nickname)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.PrimaryKey)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.Version)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.User)}""{Environment.NewLine}{'\t'}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}{'\t'}""{schema}"".""{nameof(Post)}"" a) b{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(Blog)}"" c{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Blog.PrimaryKey)}"" = b.""{nameof(Post.Blog)}""{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntities.Relations.User)}"" d{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" = b.""{nameof(Post.User)}"") e{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN @param_0 IS NULL THEN e.""{nameof(Blog)}_{nameof(Blog.Theme)}"" IS NULL ELSE e.""{nameof(Blog)}_{nameof(Blog.Theme)}"" = @param_1 END",
                        new[] { new SqlCommandParameter("param_0", "MilkyWay", typeof(string)), new SqlCommandParameter("param_1", "MilkyWay", typeof(string)) },
                        log)),
                new IDatabaseEntity[] { user, blog, post }
            };
            ...
            yield return new object[]
            {
                $"{nameof(DataAccess.Orm.Sql.Postgres)} - one-to-one relation in multi-stage projection to anonymous type",
                new Func<IDependencyContainer, object?>(container => container.Resolve<IDatabaseContext>().All<Post>().Select(it => new { it.Blog, it.User }).Select(it => new { it.Blog }).Where(it => it.Blog.Theme == "MilkyWay")),
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}b.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.PrimaryKey)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.Theme)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Post.Blog.Version)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.Nickname)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.PrimaryKey)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User.Version)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.User)}""{Environment.NewLine}{'\t'}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}{'\t'}""{schema}"".""{nameof(Post)}"" a) b{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(Blog)}"" c{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}c.""{nameof(Blog.PrimaryKey)}"" = b.""{nameof(Post.Blog)}""{Environment.NewLine}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}""{schema}"".""{nameof(DatabaseEntities.Relations.User)}"" d{Environment.NewLine}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(DatabaseEntities.Relations.User.PrimaryKey)}"" = b.""{nameof(Post.User)}"") e{Environment.NewLine}WHERE{Environment.NewLine}{'\t'}CASE WHEN @param_0 IS NULL THEN e.""{nameof(Blog)}_{nameof(Blog.Theme)}"" IS NULL ELSE e.""{nameof(Blog)}_{nameof(Blog.Theme)}"" = @param_1 END",
                        new[] { new SqlCommandParameter("param_0", "MilkyWay", typeof(string)), new SqlCommandParameter("param_1", "MilkyWay", typeof(string)) },
                        log)),
                new IDatabaseEntity[] { user, blog, post }
            };
            yield return new object[]
            {
                ...
                new Action<ICommand, ITestOutputHelper>(
                    (query, log) => CheckSqlCommand(query,
                        $@"SELECT{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"" AS ""{nameof(Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"" AS ""{nameof(Blog.Theme)}"",{Environment.NewLine}{'\t'}e.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"" AS ""{nameof(Blog.Version)}""{Environment.NewLine}FROM{Environment.NewLine}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}(SELECT{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Blog)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.DateTime)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Text)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.User)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}a.""{nameof(Post.Version)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Post.Blog.PrimaryKey)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Post.Blog.Theme)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Theme)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""{nameof(Post.Blog.Version)}"" AS ""{nameof(Post.Blog)}_{nameof(Post.Blog.Version)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(Post.User.Nickname)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Nickname)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(Post.User.PrimaryKey)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.PrimaryKey)}"",{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""{nameof(Post.User.Version)}"" AS ""{nameof(Post.User)}_{nameof(Post.User.Version)}""{Environment.NewLine}{'\t'}{'\t'}FROM{Environment.NewLine}{'\t'}{'\t'}{'\t'}""GenericHostTest"".""Post"" a{Environment.NewLine}{'\t'}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}{'\t'}""GenericHostTest"".""Blog"" b{Environment.NewLine}{'\t'}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}{'\t'}b.""PrimaryKey"" = a.""Blog""{Environment.NewLine}{'\t'}{'\t'}JOIN{Environment.NewLine}{'\t'}{'\t'}{'\t'}""GenericHostTest"".""User"" c{Environment.NewLine}{'\t'}{'\t'}ON{Environment.NewLine}{'\t'}{'\t'}{'\t'}c.""PrimaryKey"" = a.""User"") d{Environment.NewLine}{'\t'}WHERE{Environment.NewLine}{'\t'}{'\t'}d.""{nameof(Post.DateTime)}"" > @param_0) e",
                        new[] { new SqlCommandParameter("param_0", DateTime.MinValue, typeof(DateTime)) },
                        log)),
                new IDatabaseEntity[] { user, blog, post }
            };
        ...
        [Fact]
        internal void Lambda_parameter_name_increments_sequentally()
        {
            ...
            Assert.Equal("y", names[24]);
            Assert.Equal("z", names[25]);
            Assert.Equal("aa", names[26]);
            Assert.Equal("ab", names[27]);

            Assert.Equal("zy", names[700]);
            Assert.Equal("zz", names[701]);
            Assert.Equal("aaa", names[702]);
            Assert.Equal("aab", names[703]);
        }

        [Theory(Timeout = 60_000)]
        [MemberData(nameof(LinqToSqlTestData))]
        internal async Task Orm_generates_valid_queries(
            string section,
            ...
            AsyncCountdownEvent asyncCountdownEvent)
        {
            try
            {
                ...
                Assert.Equal("LinqToSqlTest", sqlDatabaseSettings.Database);
            ...
            CancellationToken token)
        {
            ...
            async Task RunWithinTransaction(IAdvancedDatabaseTransaction transaction, CancellationToken token)
            {
                ...
                output.WriteLine(item is JsonValue jsonValue
                    ? jsonValue.ToString()
                    : item.Dump(BindingFlags.Instance | BindingFlags.Public | BindingFlags.GetProperty));
            }
        }
